...
```
 ...
 @return 여기에서 획득한 값을 커멘드 핸들러에 append해야합니다.
```
## aris.init.command.suggested_string_arg(of: string, list: string) -> AbstractCommandHandler
```
 ...
 @return 여기에서 획득한 값을 커멘드 핸들러에 append해야합니다.
```
## aris.init.command.suggested_word_arg(of: string, list: string) -> AbstractCommandHandler
```
...
```
 ...
 @of 추가할 명령어 이름
```
## aris.init.command.suggestion.set_list(id: string, values: any)
```
 명령어 자동완성 리스트를 설정합니다.
 @param id 자동완성 리스트 id
 @param values 자동완성 후보 문자열 배열(table)
```
## aris.init.command.suggestion.clear_list(id: string)
```
 명령어 자동완성 리스트를 비웁니다.
 @param id 자동완성 리스트 id
```
